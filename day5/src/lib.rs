...
impl Solution {
    pub fn new() -> Self {
        Self { strings: Vec::new(), answer_part1: None, answer_part2: None }
    }

    pub fn analyse(&mut self) {
        let (nice, naughty): (Vec<_>, Vec<_>) = self.strings.iter().partition(|s| Judgement1::from_str(s).unwrap()==Judgement1::Nice);
        self.answer_part1 = Some(nice.len() as i64);

        let (nice, naughty): (Vec<_>, Vec<_>) = self.strings.iter().partition(|s| Judgement2::from_str(s).unwrap() == Judgement2::Nice);
...
impl FromStr for Judgement1 {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let mut num_vowels = Self::count_vowels(s);
        let mut contains_naughty = Self::contains_naughty_part(s);
        let mut contains_pair = Self::contains_pair(s);
    ...
    fn has_duplicate_pair(s: &str) -> bool {
        ...
        for (idx, c) in s.chars().enumerate() {
            if let Some(last_c) = last {
                pairs.entry((last_c, c)).or_insert_with(Vec::new).push(idx-1);
            }
            last = Some(c);
        }
        pairs.into_iter()
